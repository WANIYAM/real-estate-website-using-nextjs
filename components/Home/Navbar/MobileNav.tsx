  ...
  return (
    ...
    {navLinks.map((navLink) => (
            <Link key={navLink.id} href={navLink.url}>
              <p className="text-[20px] ml-12 border-b-[1.5px] pb-1 w-fit border-white sm:text-[30px] font-medium hover:text-yellow-300 cursor-pointer">
                {navLink.label}
              </p>
            </Link>
          ))}
          {/* Close button */}
    <CgClose onClick={closeNav} className='absolute top-[0.7rem] right-[1.4rem] sm:h- w-6 h-6 text-white'/>
    </div>
